...
include { PREPROCESS; SPADES; PREPPILON; PILON } from './subworkflow.nf'
...
process CHECKM {
    ...
    output:
    tuple val(reads_id),
          path("${reads_id}_bin_stats_ext.tsv"), emit: bin_stats

    script:
    """
    mkdir fasta_dir
    cp ${reads_id}.fasta fasta_dir
    ...
    mv bin_stats_ext.tsv ${reads_id}_bin_stats_ext.tsv
    """
}
...
process RAGOUTASSEMBLY {
    ...
    publishDir "${params.publish_path}${params.publish_name}", pattern: "${reads_id}_scaffolds.fasta", mode: 'copy'
    publishDir "${params.publish_path}${params.publish_name}", pattern: "${reads_id}_NCBI_SA_2025.tsv", mode: 'copy'
    ...
    tuple val(reads_id),
          ...
          path("${reads_id}_bin_stats_ext.tsv")
    val saureusref

    output:
    tuple val(reads_id),
          path ("${reads_id}_NCBI_SA_2025.tsv"), emit: qc_results
    tuple val(reads_id),
    ...
    stats=\$(grep "^${reads_id}" "${reads_id}_bin_stats_ext.tsv")
...
process MLST {
    ...
    publishDir "${params.publish_path}${params.publish_name}", pattern: "${reads_id}_MLST.txt", mode: 'copy'
    ...
    output:
    tuple val(reads_id),
          path("${reads_id}_MLST.txt"), emit: mlst_txt
...
process PHAROKKA {
    ...
    publishDir "${params.publish_path}${params.publish_name}", pattern: "${reads_id}_merged.txt", mode: 'copy'
    ...
    pharokka.py -i \${file} -o \${id}.pharokka_out -d ${pharokka_db} -t 16 --force -p \${id}
...
workflow {
    ...
    in_checkm_ch = pilon_ch.fasta
    checkm_ch = CHECKM(in_checkm_ch, params.checkm_db)

    in_ragoutassembly_ch = pilon_ch.fasta.join(spades_ch.count_qc).join(preprocess_ch.count_qc).join(checkm_ch.bin_stats)
    ragoutassembly_ch = RAGOUTASSEMBLY(in_ragoutassembly_ch, params.saureus_ref)

    in_mlst_ch = ragoutassembly_ch.scaffolds
    mlst_ch = MLST(in_mlst_ch)

    in_amrfinder_ch = ragoutassembly_ch.scaffolds
    amrfinder_ch = AMRFINDER(in_amrfinder_ch)

    in_prokka_ch = ragoutassembly_ch.scaffolds.join(mlst_ch.mlst_txt)
    prokka_ch = PROKKA(in_prokka_ch)

    in_genomad_ch = ragoutassembly_ch.scaffolds
    genomad_ch = GENOMAD(in_genomad_ch,params.genomad_db)

    in_padloc_ch = ragoutassembly_ch.scaffolds
    padloc_ch = PADLOC(in_padloc_ch)

    in_pharokka_ch = genomad_ch.provirus.join(amrfinder_ch.amr_txt).join(prokka_ch.prokka_txt).join(padloc_ch.padloc_txt)
